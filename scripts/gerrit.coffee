...
formatters =
  ...
  events:
    ...
    "comment-added":    (json) ->
      if json.approvals
        "#{extractName json} reviewed patchset #{json.patchSet.number} (#{extractReviews json}) of '#{json.change.subject}' for #{json.change.project}/#{json.change.branch}: #{json.change.url}"
      else
        "#{extractName json} added a comment to patchset #{json.patchSet.number} of '#{json.change.subject}' for #{json.change.project}/#{json.change.branch}: #{json.change.url}.\nThe comment says:\n#{json.comment}"
